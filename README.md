...
Electron/Chromium probe every GPU at launch — both via the EGL/GLX vendor
libraries *and* via a Vulkan device enumeration (the latter is the real culprit:
it loads the nvidia Vulkan ICD and wakes the dGPU even when rendering on the
iGPU). `ergctl gpu-guard on` installs `/etc/environment.d/90-ergctl-igpu.conf`
that points EGL/GLX at **mesa only** and **disables the nvidia Vulkan ICD**
(`VK_LOADER_DRIVERS_DISABLE=nvidia_icd.json`), so apps can't see or wake the
dGPU. `prime-run` still overrides GL/EGL per-process; for **Vulkan** dGPU offload
specifically, turn the guard off (`ergctl gpu-guard off`) and log back in.
Takes effect after a logout/login (then restart the apps). Enabled by
`install.sh`.
